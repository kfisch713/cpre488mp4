...
double calc_roll_gyro(int16_t x_mag, XTime t1, XTime t2);
double calc_pitch_gyro(int16_t y_mag, XTime t1, XTime t2);
...
//		pitch = calc_pitch(accy);
		roll = calc_roll_gyro(gyrx, t1, t2);
		pitch = calc_pitch_gyro(gyry, t1, t2);
...
double calc_roll_gyro(int16_t x_mag, XTime t1, XTime t2){
	static int sum = 0;
	double degree = 0;
	double diff = (t1 - t2)/(COUNTS_PER_SECOND* 1.0);
	sum += x_mag;
